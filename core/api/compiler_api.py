...
class NovelFactoryAPI:
    ...
    def use_llm_model(self, model_id: str):
        self.config.llm_model = model_id
    ...
    def plan(self, config: Optional[PipelineConfig] = None):
        ...
        if use_mock:
            ...
            llm_model = os.environ.get("NOVELFACTORY_LLM_MODEL", self.config.llm_model)
            llm = LocalLLMProvider(model_id=llm_model)
        llm.load()
