...
# Model tried automatically when the requested model does not fit in GPU VRAM.
...
class LocalLLMProvider(LLMProvider):
    ...
    def __init__(self, model_id=None):
        # Precedence: explicit arg > NOVELFACTORY_LLM_MODEL env var > default
        self.model_id = (
            model_id
            or os.environ.get("NOVELFACTORY_LLM_MODEL")
            or "Qwen/Qwen1.5-4B-Chat"
        )
        self.tokenizer = None
        self.model = None
    ...
    def load(self):
        ...
        if bnb_config:
            try:
                self.model = self._load_4bit(model_path, bnb_config, local_only)
                return
            except (ValueError, RuntimeError) as e:
                # accelerate raises ValueError when it must dispatch 4-bit modules
                # to CPU/disk (GPU VRAM too small); torch may raise a CUDA OOM.
                is_vram_issue = (
                    "dispatched on the CPU or the disk" in str(e)
                    or "out of memory" in str(e).lower()
                )
                if not is_vram_issue:
                    raise

                # ── Fallback 1: smaller model on GPU ────────────────────────
                if self.model_id != FALLBACK_MODEL_ID:
                    print(
                        f"[LLM] WARNING: Not enough GPU VRAM for {self.model_id} "
                        f"({str(e)[:120]}). "
                        f"Retrying with smaller model {FALLBACK_MODEL_ID}..."
                    )
                    ...
                    try:
                        ...
                        if (
                            "dispatched on the CPU or the disk" not in str(e2)
                            and "out of memory" not in str(e2).lower()
                        ):
                            raise

                # ── Fallback 2: CPU inference (slow but works) ───────────────
                print(
                    f"[LLM] WARNING: GPU VRAM still insufficient. Falling back to "
                    f"CPU inference ({self.model_id}, fp32) — this will be slow "
                    f"but should work."
                )
        ...
        # No bitsandbytes available: plain fp16 with auto device map
        # (GPU if present, otherwise CPU).
        self.model = AutoModelForCausalLM.from_pretrained(
            model_path,
    ...
    def _load_4bit(self, model_path, bnb_config, local_only):
