...
### Running with limited GPU VRAM

The planning LLM (`Qwen/Qwen1.5-4B-Chat`) is loaded in 4-bit, but if your GPU
cannot fit it, loading automatically falls back to a smaller model
(`Qwen/Qwen1.5-1.8B-Chat`) and finally to CPU inference, with a warning at
each step. To pin the model yourself, use the `--llm-model` flag or the
`NOVELFACTORY_LLM_MODEL` environment variable:

```bash
python main.py --novel my_script.txt --stage all --llm-model Qwen/Qwen1.5-0.5B-Chat
```
